...
<div class="card bg-light border-secondary align-self-middle p-3" id="mainBox">
  ...
  <form id="form_inp" target="{{route('adm.edit')}}" method="POST">
    @csrf
    <div class="d-flex">
      ...
      <div class="ms-5 w-50">
        ...
        <div class="input-group mb-3">
          ...
          <textarea type="text" class="form-control" id="desc-inp" rows="5" name="description"></textarea>
        </div>
        ...
        <div class="input-group mb-3">
          ...
          <input type="file" name="files" class="form-control" id="img-inp" value="" accept="image/jpeg, image/png" onchange="fileProcess(event)" >
        </div>

        <input type="hidden" value="" id="id-inp" name="product_id">
        <input type="hidden" value="" id="img-temp" name = "img">
        
      </div>
    </div>

    <div class="bottom-0 end-0 position-absolute" style="transform: translate(-90%, -75%);">
      <button class="btn btn-outline-success btn-lg align-right hideSidebar" id="confirmButton" type="submit">Confirm</button>
    </div>
      ...
      <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
        ...
        <div class="col-lg-4 d-flex align-items-stretch mb-3">
          <div class="card shadow-sm">
            <img class="img-src" src="{{ asset($product->img) }}" alt="cover" height="225" style="object-fit: contain" data-imgsrc="{{asset($product->img)}}">

            <div class="card-body d-flex flex-column">
              <b>
                <a class="card-text text-black name-src" style="text-decoration: none;">{{$product->name}}</a>
                <br>
                <a class="text-black" style="text-decoration: none;">( Rp</a>
                <a class="card-text price-src text-black price-src" style="text-decoration: none;">{{number_format($product->price, 0, ',', '.')}}</a>
              ...
              <div class="d-flex justify-content-between align-items-center mt-auto">
                <div class="btn-group">
                  <button type="button" class="btn btn-sm btn-outline-secondary showSidebar editButton" value="{{$product->id}}">Edit</button>
                  <button type="button" class="btn btn-sm btn-outline-secondary delButton" value="{{$product->id}}">Delete</button>
                </div>
                <div>
...
<script>
  ...
  for (let i = 0; i < editButton.length; i++) {
    editButton[i].addEventListener("click", () => {
      ...
      document.getElementById("price-inp").value = editParrent.querySelector('.price-src').innerHTML.match(/[0-9]/g).join("");
  ...
  // document.getElementById('img-inp').addEventListener('change', (event) => {
  //   document.getElementById("display-inp").src = document.querySelector('#img-inp').value;
  //   const fileName = url.createObjectURL()
  //   console.log(document.querySelector('#img-inp').value);
  ...
  function fileProcess(event) {
    ...
    console.log(getElementById("img-temp").value)
  }
